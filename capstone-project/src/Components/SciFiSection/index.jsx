...
export default function SciFiSection() {
    ...
    const goToMovie = (movieID) => {
        navigate(ROUTE_NAMES.DEFAULT_PAGE + movieID);
    };
            ...
            <div className="SciFiMovies">
                <div className="SciFiMovieShowcase">
                    {scifimovies.map((elem, index) => {
                        return (
                            <div onClick={() => goToMovie(elem.link)} key={index} className="SciFiMovie" id={elem.id} style={{background: `url(${elem.poster})`, backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: "cover"}}>
                                <div>
                                    <h1>{elem.name}</h1>
